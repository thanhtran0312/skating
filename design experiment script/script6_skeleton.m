...
% path 
% - set paths
% rootdir = '\\cimec-storage6.cimec.unitn.it\ingdev\projects\THANH\IceSkating'; 

% mac_dir = '/Users/goal0312/Desktop/thesis/7_experiment';
mac_dir = '\\tsclient\thesis\7_experiment';
rootdir = '\\cimec-storage6.cimec.unitn.it\ingdev\projects\THANH\IceSkating';

% rootdir =    '\\tsclient\thesis';
stimdir = fullfile(rootdir,'experiment','stimuli');
addpath(genpath(stimdir));
load(fullfile(mac_dir,"rescaled_coords.mat"),"keypoints_new")
...
load(fullfile(mac_dir,'condmat.mat')); % n_subs,n_blocks,n_runs,2
load(fullfile(mac_dir,'occlusion_matrix.mat'),'occlusion_onset'); % 52 vids x 5 timepoints of occlusion in frames
...
if runnum == 1
        ...
        % task 
        DrawFormattedText(win,'- In this experiment you will have to perform 1 of 2 possible tasks -','center', yCenter-yCenter*5/6, black);
        DrawFormattedText(win,['- The first task is that you simply pay careful attention to the posture of the skater. In this case the fixation cross is always orange-'],'center', yCenter-yCenter*4/6, black);
        ...
        % change response buttons depending on location (i.e., in MEG scanner
        % or not)
        if MEG
            DrawFormattedText(win,'- On the screen, there will be a frame showing the posture of the skater. You will need to indicate whether that was the correct posture of the skater before the occlusion. (by pressing the green button) -','center', yCenter-yCenter*1/6, black);
            DrawFormattedText(win,'- Or whether it was a different posture (by pressing the red button) -','center', yCenter, black);
        else
            DrawFormattedText(win,'- On the screen, there will be a frame showing the posture of the skater. You will need to indicate whether that was the correct posture of the skater before the occlusion. (by pressing the left arrow) -','center', yCenter-yCenter*1/6, black);
            DrawFormattedText(win,'-  Or whether it was a different posture (by pressing the right arrow) -','center', yCenter, black);
        end
        DrawFormattedText(win,'- You will now first perform a 3 minute practice block of this task before we explain the second task -','center', yCenter+yCenter*2/6, black);
...
elseif runnum == 2 % 2nd practice
        DrawFormattedText(win,['- For the second task you can completely ignore the posture and pay attention to the position of the skater. In this case the fixation cross is always green-'],'center', yCenter-yCenter*5/6, black);
        DrawFormattedText(win,'- At unexpected moments your attention will be tested: -','center', yCenter-yCenter*3/6, black);
        DrawFormattedText(win,'- The movie screen will turn black for a brief second','center', yCenter-yCenter*2/6, black);
        if MEG
            DrawFormattedText(win,'- On the screen, there will be a frame showing the position of the skater. You will need to indicate whether that was the correct position of the skater before the occlusion. (by pressing the green button) -','center', yCenter-yCenter*1/6, black);
            DrawFormattedText(win,'- Or whether it was at a different position (by pressing the red button) -','center', yCenter, black);
        else
            DrawFormattedText(win,'- On the screen, there will be a frame showing the position of the skater. You will need to indicate whether that was the correct position  of the skater before the occlusion. (by pressing the left arrow) -','center', yCenter-yCenter*1/6, black);
            DrawFormattedText(win,'-  Or whether it was at a different position (by pressing the right arrow) -','center', yCenter, black);
        end
    
        DrawFormattedText(win,'- You will now first perform a practice block of this task before we continue -','center', yCenter+yCenter*2/6, black);
        DrawFormattedText(win,'- Inform the experimenter you have read the instructions -','center', yCenter+yCenter*5/6, black);
...
elseif runnum > 2
        DrawFormattedText(win,'In this run, posture and position tasks will be mixed. Before each trial, you will be informed if you should focus on the posture or position of the skater. The color of the fixation cross also indicates the task of the trial, with orange for posture and green for position.','center', yCenter-yCenter*4/6, black);
end
...
for itrial = 1:size(condmat,2) % condmat = 30 participants x 6 trials x 6 runs x 2 vid&cond
   %% movie for the condition
    if any(currentcondition(itrial) == [1,4])
            movienames = sprintf('%s%s%s%schunk%d.mp4',stimdir,filesep,'normal',filesep,vid2disp(itrial));
    elseif any(currentcondition(itrial) == [2,5])
    ...
    idx = randperm(5);
    idx = sort(idx(1:2));
    occ_onset = occlusion_onset(condmat(subnum,itrial,runnum,1),idx); % 2 frames of occlusion onset
...
%     % - **show frame according to condition 123 - posture vs 456 - position
             if any(currentcondition(itrial) == [1 2 3])
                if MEG
                    DrawFormattedText(win, '-Press the orange button if you think this is the same posture as before the occlusion,','center', yCenter-yCenter*2/6, black);
                    DrawFormattedText(win, '-Or the green button if you think the frame shows a different posture-','center', yCenter-yCenter*1/6, black);
                else
                    DrawFormattedText(win, '-Press the right arrow if you think this is the same posture before the occlusion,','center', yCenter-yCenter*1/6, black);
                    DrawFormattedText(win, '-Or the left arrow if you think the frame shows a different posture-','center', yCenter-yCenter*1/6, black);
                end
                Screen('Flip',win);
                WaitSecs(0.5);
                 %    show posture/coherent or incoherent
                if correct_response(occ_count_across_trials) == 1 % coherent -> show the frame after 5 frames
                ...
                elseif correct_response(occ_count_across_trials) == 0 
                    if currentcondition(itrial) == 1
                        tmp_path = fullfile(mac_dir,'posture_incoherent_normal');
                        frametoshow = sprintf("occ_vid_%d_time_%d_.png", vid2disp(itrial), idx(occ_count_within_trial));
                    elseif currentcondition(itrial) == 2
                        tmp_path = fullfile(mac_dir,'posture_incoherent_low');
                        frametoshow = sprintf("low_occ_vid_%d_time_%d.png", vid2disp(itrial), idx(occ_count_within_trial));
                    elseif currentcondition(itrial) == 3
                        tmp_path = fullfile(mac_dir,'posture_incoherent_high');
                        frametoshow = sprintf("high_occ_vid_%d_time_%d.png", vid2disp(itrial), idx(occ_count_within_trial));
                    end
             ...
             elseif any(currentcondition(itrial) == [4 5 6])
                ...
                if MEG
                    DrawFormattedText(win, '-Press the orange button if you think the dot is at the same position before the occlusion,','center', yCenter-yCenter*2/6, black);
                    DrawFormattedText(win, '-Or the green button if you think the dot is at a different position-','center', yCenter-yCenter*1/6, black);
                else
                    DrawFormattedText(win, '-Press the right arrow if you think the dot is at the same position before the occlusion,','center', yCenter-yCenter*1/6, black);
                    DrawFormattedText(win, '-Or the left arrow if you think the dot is at a different position-','center', yCenter-yCenter*1/6, black);
                end
                Screen('Flip',win);
                WaitSecs(0.5);

                if correct_response(occ_count_across_trials) == 1
                ...
                Screen('FillRect', win, [255 0 0], dotRect);
             end
             Screen('Flip', win);
%     % - collect response
...
%     % - rewinde to 1s before occlusion & update parameters 
            ...
            occ_count_across_trials = occ_count_across_trials + 1;
       % restart the movie
